"use client";

import { Button } from "@/components/ui/Button";
import { CTA } from "@/lib/cta";
import { estimateHref } from "@/lib/nav";
import { motion } from "framer-motion";
import { ArrowRight } from "lucide-react";

type PageCtaProps = {
  title?: string;
  description?: string;
  /** Render only the CTA card — for grouping with adjacent blocks on a page */
  embedded?: boolean;
};

export function PageCta({
  title = "Ready to reach every door in your market?",
  description = "Get a free estimate in under 60 seconds. A real person follows up within one business day.",
  embedded = false,
}: PageCtaProps) {
  const card = (
    <motion.div
      initial={{ opacity: 0, y: 20 }}
      whileInView={{ opacity: 1, y: 0 }}
      viewport={{ once: true }}
      className="relative rounded-3xl sm:rounded-[2rem] overflow-hidden px-6 py-12 sm:px-16 sm:py-16 text-center bg-brand-primary shadow-xl shadow-brand-primary/25"
    >
      <div className="absolute inset-0 bg-[url('data:image/svg+xml;base64,')] opacity-50" />
      <div className="relative">
        <h2 className="text-2xl sm:text-3xl font-semibold text-white tracking-tight mb-3">
          {title}
        </h2>
        <p className="text-white/90 text-base sm:text-lg max-w-xl mx-auto mb-8 leading-relaxed">
          {description}
        </p>
        <Button
          href={estimateHref()}
          size="lg"
          variant="inverse"
          className="w-full sm:w-auto"
        >
          {CTA.estimate}
          <ArrowRight className="h-5 w-5 shrink-0" />
        </Button>
      </div>
    </motion.div>
  );

  if (embedded) {
    return card;
  }

  return (
    <section className="section-padding bg-background">
      <div className="container-narrow">{card}</div>
    </section>
  );
}